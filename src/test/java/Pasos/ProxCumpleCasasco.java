...
public class ProxCumpleCasasco {
	AndroidDriver driver;
	
	public void pasosProxCumpleCasasco(Datasources data, Reports report, Repo_Template repo, DriverManager DM, int iteration, String name, String ocasion) throws InterruptedException {
		driver = (AndroidDriver) DM.getActualDriver();
		DateTimeFormatter dtf = DateTimeFormatter.ofPattern("dd/MMMM/yyy");
		
		TouchAction ta = new TouchAction(driver);
		ta.press(point(557,557)).moveTo(point(615,615)).release().perform();
		...
		action.scroll(repo.get_btn_prox_cumple(), 300, 300);
		action.perform();
